use crate::db::schema::r_tracks::dsl::*;
use crate::db::tracks::{Track, TrackInput};
use diesel::prelude::*;
use diesel::result::Error;
use diesel::sql_types::Integer;
use diesel::SelectableHelper;
use diesel_async::RunQueryDsl;
use diesel_async::{AsyncConnection, AsyncMysqlConnection};

pub(crate) async fn get_all_by_user_id(
    user_id: i32,
    conn: &mut AsyncMysqlConnection,
) -> Result<Vec<Track>, Error> {
    r_tracks
        .filter(uid.eq(user_id))
        .select(Track::as_select())
        .load(conn)
        .await
}

pub(crate) async fn get_one_by_id_and_user_id(
    id: i32,
    user_id: i32,
    conn: &mut AsyncMysqlConnection,
) -> Result<Option<Track>, Error> {
    match r_tracks
        .filter(tid.eq(id))
        .filter(uid.eq(user_id))
        .select(Track::as_select())
        .first(conn)
        .await
    {
        Ok(channel) => Ok(Some(channel)),
        Err(Error::NotFound) => Ok(None),
        Err(e) => Err(e),
    }
}

pub(crate) async fn create(
    track_input: &TrackInput,
    user_id: i32,
    conn: &mut AsyncMysqlConnection,
) -> Result<Track, Error> {
    conn.transaction::<_, Error, _>(|trans| {
        Box::pin(async move {
            diesel::insert_into(r_tracks)
                .values((uid.eq(&user_id), track_input))
                .execute(trans)
                .await?;

            let last_id: i32 = diesel::dsl::sql::<Integer>("SELECT LAST_INSERT_ID()")
                .get_result(trans)
                .await?;

            let inserted_track = r_tracks
                .filter(tid.eq(last_id))
                .select(Track::as_select())
                .first(trans)
                .await?;

            Ok(inserted_track)
        })
    })
    .await
}

pub(crate) async fn update(
    track_input: &TrackInput,
    track_id: i32,
    user_id: i32,
    conn: &mut AsyncMysqlConnection,
) -> Result<Option<Track>, Error> {
    conn.transaction::<_, Error, _>(|trans| {
        Box::pin(async move {
            let rows_updated = diesel::update(r_tracks)
                .filter(tid.eq(track_id))
                .filter(uid.eq(user_id))
                .set(track_input)
                .execute(trans)
                .await?;

            if rows_updated == 0 {
                return Ok(None);
            }

            let updated_track = r_tracks
                .filter(tid.eq(track_id))
                .filter(uid.eq(user_id))
                .select(Track::as_select())
                .first(trans)
                .await?;

            Ok(Some(updated_track))
        })
    })
    .await
}

pub(crate) async fn delete(
    track_id: i32,
    user_id: i32,
    conn: &mut AsyncMysqlConnection,
) -> Result<bool, Error> {
    let rows_deleted = diesel::delete(r_tracks)
        .filter(tid.eq(track_id))
        .filter(uid.eq(user_id))
        .execute(conn)
        .await?;

    Ok(rows_deleted == 1)
}
